...
// This file contains the core memeory handling functions, including implementastion of paging, 
// heap management, and memory manipulation functions.
...
// Structure of the allocation metadata
...
//Debugging function for when the kernel runs out of memory. It will print a message and halt the system.
...
// Print the current memory metadata, including used memory, 
// free memory, heap size, and the addresses of the heap and page-aligned heap.
void print_memory_layout()
{
    ...
    printf("Memory free: ");
    print_hex(heap_end - heap_begin - memory_used);
    printf("bytes\n");
    
    printf("Heap size: ");
    print_hex(heap_end - heap_begin);
    printf("bytes\n");

    printf("Heap begin: 0x");
...
char* pmalloc(size_t size)
{
    ...
    for(int i = 0; i < MAX_PAGE_ALIGNED_ALLOCS; i++)
    {
        ...
        pheap_desc[i] = 1;
        printf("PAllocated from ");
        print_hex(pheap_begin + i * 4096);
        printf(" to ");
        print_hex(pheap_begin + (i + 1) * 4096);
        printf("\n");
...
void* malloc(size_t size)
{
    ...
    while((uint32_t)mem < last_alloc)
    {
        ...
        if(a->size >= size)
        {
            ...
            printf(" to ");
            print_hex((uint32_t)mem + sizeof(alloc_t) + size);
            printf("\n");
...
//=========================== End of memory manipulation functions ===========================

//=========================== Copy Memeory Function  ===========================
