...
/*
 * This file applies movement and turning to the player
 * W and S move forward and backward
 * A and D strafe
 * Q and E turn the player
 */

void raycaster_apply_key_state_internal(Raycaster *rc, uint8_t *key_down)
{
    // Turn the key array into simple direction values
    ...
    if (turn_axis != 0) {
        // Keep the angle inside the normal 0 to 359 range
    ...
    // Start from the current position and add movement to it
