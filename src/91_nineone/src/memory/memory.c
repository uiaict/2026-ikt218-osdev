...
static uint32* page_directory = 0;   // Define a pointer to the page directory and initialize it to zero
static uint32 page_dir_loc = 0;      // Define the location of the page directory and initialize it to zero
static uint32* last_page = 0;        // Define a pointer to the last page and initialize it to zero
...
void paging_map_virtual_to_phys(uint32 virt, uint32 phys)
{
    ...
    page_directory[id] = ((uint32)last_page) | 3;  // Set the page directory entry for the virtual address to the physical address of the page table with present and write permissions set
    last_page = (uint32 *)(((uint32)last_page) + 4096); // Move to the next page in memory
}
