...
void init_kernel_memory(uint32_t* kernel_end)
{
    ...
    heap_end   = (uint32_t*)((uint32_t)heap_begin + 0x100000); // 1 MB heap
    pheap_begin = heap_end;
    pheap_end   = (uint32_t*)((uint32_t)pheap_begin + MAX_PAGE_ALIGNED_ALLOCS * 4096);
    memset((char *)heap_begin, 0, (uint32_t)heap_end - (uint32_t)heap_begin);
