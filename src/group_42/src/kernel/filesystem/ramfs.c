...
static int ramfs_driver_open(vfs_fd_t* fd, const char* path) {
    ...
    if (!node) {
        ...
        if (fd->flags & O_CREAT)
        {
            ...
            // 2. Allocate and initialize the new inode
            ...
            // 3. Link the new node into the parent's child list
...
static int ramfs_driver_read(vfs_fd_t* fd, void* buf, size_t n) {
    ram_inode_t* inode = (ram_inode_t*)fd->private_data;

    // 1. Safety check: Can't read a directory like a file
    ...
    // 2. EOF Check: Are we already at or past the end?
    if (fd->offset >= inode->size) return 0;

    // 3. Clamp 'n': Don't read past the end of the file
    if (fd->offset + n > inode->size) {
        n = inode->size - fd->offset;
    }

    // 4. Perform the copy
    memcpy(buf, (uint8_t*)inode->data + fd->offset, n);
...
static int ramfs_driver_write(vfs_fd_t* fd, const void* buf, size_t n) {
    ...
    // 1. Expansion Logic: Do we need more memory?
    ...
    // 3. Update size if we extended the file
    if (fd->offset + n > inode->size) {
...
static vfs_driver_t ram_driver = {
    .read  = ramfs_driver_read,  // You'll implement this next
    .write = ramfs_driver_write, // And this
    ...
    // ... stub the rest with NULL or a "not supported" function
};
