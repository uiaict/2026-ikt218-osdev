...
/* A single musical note: a frequency in Hz and a duration in milliseconds.
   R (frequency = 0) represents a rest (silence). */
typedef struct {
    uint32_t frequency;
    uint32_t duration;
} Note;

/* A song is an array of notes plus its length. */
typedef struct {
    Note    *notes;
    uint32_t length;
} Song;

/* A song player holds a function pointer so the caller doesn't need to
   know about the underlying hardware driver. */
typedef struct {
