...
{
    // FIX: kernel_end is a pointer to an address, so do arithmetic on the address value
...
void free(void *mem)
{
    ...
    // FIX: avoid void* arithmetic, and keep accounting consistent (+4 padding)
...
void* malloc(size_t size)
{
    ...
    while((uint32_t)mem < last_alloc)
    {
        ...
        if(a->size >= size)
        {
            ...
            // FIX: keep size/accounting consistent with free()
            a->size = size;

            printf("RE:Allocated %d bytes from 0x%x to 0x%x\n",
                   (int)size, mem + sizeof(alloc_t), mem + sizeof(alloc_t) + size);
...
nalloc:;
    ...
    printf("Allocated %d bytes from 0x%x to 0x%x\n",
           (int)size, (uint32_t)alloc + sizeof(alloc_t), last_alloc);
